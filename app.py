...
def get_elliott_wave(df):
    ...
    changes = close.diff().dropna(); waves=[]; current=0
    for c in changes:
        if c>0:
...
def get_stock_data(symbol, is_crypto=False):
    try:
        ...
        bb_upper=df["BB_upper"].iloc[-1]; bb_lower=df["BB_lower"].iloc[-1]; chg=((price-prev)/prev)*100
        resistances,supports=get_support_resistance_levels(df); elliott=get_elliott_wave(df)
        ...
        return {"symbol":symbol,"price":round(price,d),"change_pct":round(chg,2),"arrow":"▲" if chg>=0 else "▼",
                ...
                "resistances":resistances,"supports":supports,"elliott":elliott,"is_crypto":is_crypto,"close_series":df["Close"]}
...
def get_weekly():
    ...
    fire_air=["Aries","Leo","Sagittarius","Gemini","Libra","Aquarius"]; result=[]
    for i in range(7):
...
def analyze(stock,astro_str,sign_th,sign_en,weekly,birth,tarot_cards,api_key):
    ...
    prompt=f"""คุณคือ AstroTrade AI นักวิเคราะห์ที่เท่ พูดภาษาไทยเข้าใจง่าย
...
ตัดขาดทุน: {stock['sl']} | แนวต้าน: {stock['resistances']} | แนวรับ: {stock['supports']}
ไพ่ยิปซี: {tarot_str} | ดาววันนี้: {astro_str} | {birth_info}
วันดี/ร้าย: {weekly_str}
วิเคราะห์: 1.📊ภาพรวม 2.🌊Elliott Wave 3.🎯ซื้อ/ขาย/ตัดขาดทุนที่ไหน 4.🃏ไพ่ยิปซีบอกอะไร 5.🪐ดาว+ดวงชะตา(สนุกๆ) 6.📅วันดีที่สุด 7.✨คำคมปิดท้าย
...
if hot:
    ...
    for i,s in enumerate(hot):
        rank=["🥇","🥈","🥉","4️⃣","5️⃣"][i]; clr="#00e676" if s["change_pct"]>=0 else "#ff5252"
        bar=min(int((s["score"]/10)*100),100)
        st.markdown(f"""<div class="poll-card"><div style='display:flex;justify-content:space-between;align-items:center'><div><span style='font-size:1.2em'>{rank}</span> <b style='font-size:1.05em;margin-left:6px'>{s['symbol']}</b> <span style='color:#888;font-size:0.8em'>RSI {s['rsi']}</span></div><div style='text-align:right'><b>{s['price']}</b><br><span style='color:{clr};font-size:0.9em'>{'▲' if s['change_pct']>=0 else '▼'}{abs(s['change_pct'])}%</span> <span style='color:#666;font-size:0.8em'>Vol×{s['vol_ratio']}</span></div></div><div class="poll-bar-bg"><div style='width:{bar}%;height:8px;border-radius:20px;background:linear-gradient(90deg,#7c4dff,#e040fb)'></div></div></div>""", unsafe_allow_html=True)
...
if is_crypto:
    ...
    with c2: manual=st.text_input("พิมพ์ชื่อ",placeholder="เช่น SOL-USD") if CRYPTO_LIST[sel]=="CUSTOM" else st.empty() and ""
    ...
    with c2: manual=st.text_input("พิมพ์ชื่อ",placeholder="เช่น NVDA") if STOCK_LIST[sel]=="CUSTOM" else st.empty() and ""
...
# ============================================================
# UI: สำรับไพ่ยิปซี — Fan/Arc style
# ============================================================
...
if st.button("🔀 สับไพ่ใหม่"):
    ...
    st.session_state.shuffled_deck=deck; st.session_state.selected_indices=[]; st.rerun()

deck=st.session_state.shuffled_deck
selected=st.session_state.selected_indices

# สร้าง HTML สำรับไพ่แบบ Arc/Fan
cards_json=json.dumps([{"name":c["name"],"th":c["th"],"emoji":c["emoji"],"meaning":c["meaning"]} for c in deck])
selected_json=json.dumps(selected)

fan_html = f"""
<div id="tarot-container" style="position:relative;width:100%;padding-bottom:20px;">

  <!-- สำรับไพ่ Arc -->
  <div id="fan-wrap" style="position:relative;height:280px;width:100%;overflow:visible;margin-bottom:10px;">
    <div id="fan-center" style="position:absolute;bottom:0;left:50%;transform:translateX(-50%);width:0;height:0;"></div>
  </div>

  <!-- ไพ่ที่เลือกแล้ว -->
  <div id="selected-wrap" style="display:flex;gap:12px;justify-content:center;margin-top:8px;flex-wrap:wrap;min-height:140px;">
    <div style="color:#666;font-size:0.85em;align-self:center" id="placeholder-text">กดไพ่จากสำรับด้านบน</div>
  </div>

  <!-- ปุ่มล้าง -->
  <div style="text-align:center;margin-top:10px">
    <button onclick="clearCards()" style="background:rgba(124,77,255,0.2);border:1px solid #7c4dff;color:#e040fb;border-radius:20px;padding:6px 20px;cursor:pointer;font-size:0.85em;">✕ ล้างไพ่</button>
  </div>

  <!-- hidden input ส่งค่ากลับ -->
  <input type="hidden" id="selected-output" value="{selected_json}">
</div>
...
.tarot-fan-card {{
  ...
  background: linear-gradient(145deg, #2d0a4e, #1a0533);
...
.tarot-fan-card:hover {{
  box-shadow: 0 0 25px rgba(224,64,251,0.8);
...
.tarot-fan-card .card-star {{
  font-size:1.4em;
}}
.tarot-fan-card .card-label {{
  font-size:0.42em;
  color:#9c27b0;
...
.selected-card .remove-btn {{
  ...
  background:none;border:none;color:#888;cursor:pointer;font-size:0.8em;
}}
</style>
...
function renderFan() {{
  var wrap = document.getElementById('fan-wrap');
  // ลบไพ่เก่า
  var old = wrap.querySelectorAll('.tarot-fan-card');
  old.forEach(function(el){{el.remove();}});

  var centerX = wrap.offsetWidth / 2;
  var radius = Math.min(wrap.offsetWidth * 0.48, 220);
  var startAngle = -80;
  var endAngle = -100;
  var totalAngle = 160;
  var step = totalAngle / (total - 1);

  for (var i = 0; i < total; i++) {{
    var angle = startAngle + i * step - 90;
    var rad = angle * Math.PI / 180;
    var x = centerX + radius * Math.cos(rad) - 26;
    var y = 260 + radius * Math.sin(rad) - 41;
    var rotDeg = angle + 90;
    ...
    card.innerHTML = '<div class="card-star">🌟</div><div class="card-label">TAROT</div>';
    card.dataset.idx = i;
    ...
    card.addEventListener('click', function() {{
      ...
      selected.push(idx);
      document.getElementById('selected-output').value = JSON.stringify(selected);
      renderFan();
      renderSelected();
    }});
...
function renderSelected() {{
  var wrap = document.getElementById('selected-wrap');
  var placeholder = document.getElementById('placeholder-text');
  if (selected.length === 0) {{
    wrap.innerHTML = '<div style="color:#666;font-size:0.85em;align-self:center" id="placeholder-text">กดไพ่จากสำรับด้านบน</div>';
    return;
  }}
  wrap.innerHTML = '';
  for (var i = 0; i < selected.length; i++) {{
    ...
    div.dataset.selIdx = i;
    div.innerHTML =
      '<div class="role-label">' + role + '</div>' +
      '<div class="card-emoji">' + c.emoji + '</div>' +
      '<div class="card-name">' + c.th + '</div>' +
  ...
  countDiv.style.cssText = 'width:100%;text-align:center;color:#e040fb;font-size:0.85em;margin-top:4px';
  countDiv.textContent = '✨ เลือกแล้ว ' + selected.length + '/3 ใบ';
  wrap.appendChild(countDiv);
}}

function removeCard(selIdx) {{
...
setTimeout(function(){{ renderFan(); renderSelected(); }}, 300);
</script>
"""

st.components.v1.html(fan_html, height=480, scrolling=False)

# อ่านค่าไพ่ที่เลือกจาก session state (ใช้ st.session_state แทน JS)
st.markdown("<div style='color:#b39ddb;font-size:0.8em;margin-top:-8px;margin-bottom:8px'>⚡ หลังเลือกไพ่ครบ 3 ใบแล้ว กดปุ่มยืนยันด้านล่าง</div>", unsafe_allow_html=True)

# เลือกไพ่ผ่าน multiselect เป็น backup ที่ sync กับ fan
card_names = [f"{c['emoji']} {c['th']}" for c in TAROT_CARDS]
selected_names = st.multiselect(
    "หรือเลือกไพ่จากรายการ (เลือก 3 ใบ)",
    card_names,
    max_selections=3,
    placeholder="เลือกไพ่ยิปซี 3 ใบ..."
)

# แปลงชื่อกลับเป็น card objects
selected_cards = []
...
if selected_cards:
    ...
    st.markdown(f"<div style='color:#e040fb;font-size:0.9em;margin:8px 0'>✨ เลือกแล้ว {len(selected_cards)}/3 ใบ</div>", unsafe_allow_html=True)
        ...
        for i, card in enumerate(selected_cards):
            with tc[i]:
                ...
                <div class="card-revealed">
                    ...
                    <div style='color:#ce93d8;font-size:0.68em;letter-spacing:1px'>{roles[i]}</div>
...
if run:
    if not GEMINI_KEY:
        ...
        st.warning(f"⚠️ เลือกไพ่ให้ครบ 3 ใบก่อนนะครับ (เลือกแล้ว {len(selected_cards)} ใบ)")
    else:
        roles = ["สถานการณ์ปัจจุบัน", "แนวโน้มระยะสั้น", "คำแนะนำ"]
        ...
        if not stock:
            ...
            <div class="{stock['cls']}">
                ...
                <table style='width:100%;color:#eee;font-size:0.92em'>
                    <tr><td style='padding:5px 0'>🎯 TP1 (ระยะสั้น)</td><td style='text-align:right'><b style='color:#69f0ae'>{stock['tp1']}</b></td></tr>
                    <tr><td style='padding:5px 0'>🎯 TP2</td><td style='text-align:right'><b style='color:#40c4ff'>{stock['tp2']}</b></td></tr>
                    <tr><td style='padding:5px 0'>🎯 TP3</td><td style='text-align:right'><b style='color:#e040fb'>{stock['tp3']}</b></td></tr>
                    <tr><td style='padding:5px 0'>🎯 TP4 (ระยะยาว)</td><td style='text-align:right'><b style='color:#FFD700'>{stock['tp4']}</b></td></tr>
                    <tr><td style='padding:5px 0'>🛑 ตัดขาดทุน</td><td style='text-align:right'><b style='color:#ff6b6b'>{stock['sl']}</b></td></tr>
                </table>
            ...
            if sign_th:
                st.markdown(f"""<div class="glass-card" style='text-align:center'><div style='font-family:Playfair Display,serif;color:#FFD700;font-size:1.1em;font-weight:700'>♈ ราศีเกิดของคุณ</div><div style='font-size:1.8em;font-weight:900;background:linear-gradient(90deg,#FFD700,#FFA500);-webkit-background-clip:text;-webkit-text-fill-color:transparent'>{sign_th} ({sign_en})</div></div>""", unsafe_allow_html=True)
            else:
